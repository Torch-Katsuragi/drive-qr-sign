...
⚠**これを飛ばすと、記録メールが迷惑メールに入る。** 実際に入った。

送信サービスが提示する DNS レコードのうち、DMARC は「任意」「推奨」の扱いで出てくることが多い。
だが DKIM と SPF だけではポリシーが無い状態で、Gmail は新しい送信ドメインをまず疑う。
控えが迷惑メールに落ちると、**署名者の手元に記録が残る**という記録メールの狙いが立たない。

```
TXT  _dmarc.<送信サブドメイン>  v=DMARC1; p=none;
```

⚠**置き場所を間違えない。** 送信サービスは `_dmarc`（＝**組織ドメイン直下**）を提示してくることがある。
それを入れると、そのドメイン全体——業務用のメールを含む——にポリシーが乗る。

DMARC は `_dmarc.<From のドメイン>` を先に引き、無ければ組織ドメインへ落ちる（RFC 7489）。
つまり送信を `send.example.com` のようなサブドメインに閉じているなら、**`_dmarc.send` に置けば
そのサブドメインにだけポリシーが付く**。既存のメール設定には触らずに済む。
...
| 署名ページが 403「この URL は無効です」 | QR の鍵が食い違っている。Secret Manager の値に改行が混ざっていないか見る（→ 5章）。アプリ側は前後の空白を落として突き合わせるので、いま作る QR では起きない |
| デプロイは通るのに誰も署名ページを開けない | `allUsers` が付いていない。組織ポリシーの「ドメイン制限共有」を疑う（→ 5章） |
| 署名が 409「ほかの人の署名と重なりました」 | 同時に押された。押し直せばよい（先の署名は消えていない） |
| 記録メールが迷惑メールに入る | 送信ドメインに DMARC が無い。`_dmarc.<送信サブドメイン>` を置く（→ 7章） |
